...
const PaperPreviewModal: React.FC<PaperPreviewModalProps> = ({
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[90vh] p-0 w-[95vw] sm:w-full flex flex-col overflow-hidden">
                  ...
                  <div className="space-y-4">
                    {questionsToPreview.map((question, index) => (
                      <div key={question.id} className="border-l-4 border-blue-600 pl-3 sm:pl-4">
                        ...
                        {(() => {
                          const rawImageUrl = (question as any).mediaUrl || (question as any).imageUrl || (question as any).image;
                          if (!rawImageUrl) return null;
                          // Fix relative URLs by prepending base URL
                          const ADMIN_BASE_URL = "https://08m8v685-3002.inc1.devtunnels.ms";
                          const imageUrl = rawImageUrl.startsWith('http') ? rawImageUrl : `${ADMIN_BASE_URL}${rawImageUrl.startsWith('/') ? '' : '/'}${rawImageUrl}`;
                          return (
                            <div className="mb-2">
                              <img 
                                src={imageUrl} 
        ...
        {/* Action Buttons */}
        <div className="p-4 sm:p-6 pt-4 border-t bg-gray-50 dark:bg-gray-800 flex-shrink-0">
